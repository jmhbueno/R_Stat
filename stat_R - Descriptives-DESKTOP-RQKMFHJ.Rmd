...
Duas formas de apresentação de estatística descritiva muito comuns são os formatos de tabelas e gráficos.
...
## colorindo as barras por país
#obs: Por padrão, geom_bar usa stat_count() para determinar o tamanho das barras. geom_col é usado quando queremos que as barras representem valores diretamente presentes no conjunto de dados. Por padrão, geom_col usa stat_identity() para determinar o tamanho das barras. Então nesse caso, estamos usando o geom_col, mas, para isso, temos que produzir a tabela de dados primeiro.

dataset %>% 
  count(country) %>% 
  mutate(pct = n/sum(n)) %>% 
  ggplot(.,aes(x = country, y = pct, fill = country)) + 
...
ggplot(dataset, 
  ...
  geom_bar(stat = "summary", 
           fun = mean, 
           color = "black",
           width = 0.7,      # diminuir um pouco a espessura das barras
           na.rm = TRUE) + 
...
ggplot(dataset, aes(x = country, y = bai_sum, fill = country)) +
