...
qnorm(new_alfa/2)   # z relativo ao new_alfa: > 2,64 ou < -2,64, para alfa=0,83%
                    # Usar esse valor para avaliar os resíduos padr. ajustados
...
Nesse caso, o valor de p (3.003e-11) foi menor que 0.008 (novo valor referência de p calculado), indicando que o sexo dos participantes é distribuído de forma desigual entre os países. No entanto, o tamanho do efeito (V_Cramer = 0.1576) para 2 graus de liberdade, foi médio.
...
3.  Calcular o qui-quadrado para verificar as associações entre país e ansiedade (bai_class)

4.  Calcular o qui-quadrado para verificar as associações entre país e depressão (bai_class)
...
## também roda na sintaxe tidyverse
matriz_psych <- big_five %>% select(59:63) %>% corr.test(method = "pearson")

matriz_psych$r
matriz_psych$n
matriz_psych$p
matriz_psych$stars %>% kable()
...
# Fidedignidade
```{r}
dataset %>% select(46:66) %>% omega(poly = TRUE)
dataset %>% select(47:87) %>% omega(poly = TRUE)
```
